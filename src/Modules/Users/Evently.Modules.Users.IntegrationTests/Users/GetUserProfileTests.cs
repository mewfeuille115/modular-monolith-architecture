...
public class GetUserProfileTests : BaseIntegrationTest
{
	...
	{
		// Act
		HttpResponseMessage response = await HttpClient.GetAsync("users/profile", CancellationToken.None);
	...
	public async Task Should_ReturnOk_WhenUserExists()
	{
		...
		UserResponse? user = await response.Content.ReadFromJsonAsync<UserResponse>(CancellationToken.None);
